...
#Versao 6 -> Grava os valores em um txt
#Versao 7 -> Variar permeabilidade tambem
...
u_a = 1.1 - 0.05j    #Analitico teflon
#------------------------------------------------------------
...
X_por_f_ur = [] #Melhor X por frequencia para permissividade real
X_por_f_ui = [] #Melhor X por frequencia para permissividade imaginaria



e1_x = np.arange(22,24.6,0.2)
e2_x = np.arange(2,5,0.2)
...
u1_x = np.arange(0,2,1)
u2_x = np.arange(0,0.02,0.01)


u_x=[]

for i in range(0,len(u1_x)):
	
	for j in range(0,len(u2_x)):
		
		u_x.append(u1_x[i]-1j*u2_x[j])
...
for i in range(0,len(ler)):
	...
	e_u = [] #Quardar os valores quando calcular o chi
	...
	#VARIAR PERMEABILIDADE
	for p in range(0,len(u_x)):	
	...
	for n in range(0,len(X_s11_s21)):
		...
		if (x_s11_s21_normalizado == 1.0):
			...
			X_por_f_er.append(e_a.real)
			X_por_f_ei.append(e_a.imag*(-1))
			
			X_por_f_ur.append(u_a.real)
			X_por_f_ui.append(u_a.imag*(-1))
...
plt.plot(F_grafic,X_por_f_ur,'g-',label ="ur",alpha=1)
plt.plot(F_grafic,X_por_f_ui,'b-',label="ui",alpha=1)
plt.xlim(8.2,12.4)
#plt.ylim(-0.25,2.5)
plt.xlabel("Freq(GHz)")
plt.ylabel("S11,S21(a.u)")
...
plt.plot(F_grafic,s11c_teo_mod,'k-',label ="s11_linear_teo",alpha=1)
plt.plot(F_grafic,s21c_teo_mod,'r-',label="s21_linear_teo",alpha=1)
...
arqnew= open("./_calculados"+TXT[0][:-4]+'.txt','w')

arqnew.write("Fre(Hz)  	er	ei	ur	ui\n")

for n in range(0,len(F)):

	escrever = "%.4f	%.4f	%.4f	%.4f	%.4f\n"%(F[n],X_por_f_er[n],X_por_f_ei[n],X_por_f_ur[n],X_por_f_ui[n])
