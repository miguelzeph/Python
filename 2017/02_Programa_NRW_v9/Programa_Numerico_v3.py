...
e1_x = np.arange(20,30,1)
e2_x = np.arange(0,1,0.5)
...
for i in range(0,len(ler)):
	...
	for w in range(0,len(e_x)):
		...
		s11c_teo_mod.append(abs(s11c_teo[w]))
		s21c_teo_mod.append(abs(s21c_teo[w]))
		
		
		
		#Chi Quadrado S11 
		chi_quad_s11 = (abs(s11c_teo[w])- abs(s11c[i]))**(2.0)/abs(s11c[i])
		#Chi Quadrado S21
		chi_quad_s21 = (abs(s21c_teo[w])- abs(s21c[i]))**(2.0)/abs(s21c[i])
