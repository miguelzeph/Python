...
ground=box(pos=(0,-1,0),size=(100,1,50))
...
while not finished:
	...
	alcance=-50+vel_total.x*seconds 
	
	altura= 0+vel_total.y*seconds-0.5*9.8*seconds**2
	largura= vel_total.z*seconds
	#fazer variar o vetor em x e y...
	ball.pos=vector(alcance,altura,largura)
	...
	if altura <=0:#quando altura for igual ou menor a 0 , perceba q esta dentro do loop , entao no primeiro zero ele n ira parar...
		...
		largura_max=largura-(0)
		#aplicar pitagoras para saber deslocamento horizontal,tente visualisar um triangulo olhando de cima...
		diagonal_max=(alcance_max**2+largura_max**2)**(0.5)
		
		
		
		label(pos=(0,20,0),text='alcance maximo='+str(alcance_max))
		label(pos=(0,10,0),text='largura maximo='+str(largura_max))
		label(pos=(0,30,0),text='aplicando pitagoras, sabemos o deslocamento diagonal maximo='+str(diagonal_max),color=color.blue)