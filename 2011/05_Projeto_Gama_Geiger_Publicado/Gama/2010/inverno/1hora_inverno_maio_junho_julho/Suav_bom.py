...
for n in ('10','60'):
	...
	for i in range(0,len(lista)):
		...
		for p in range(0,len(txts)):
			...
			for linha in range(26,len(arq_dados)-1):
				...
				if (float(dose) >= 0.05 and float(dose) <= 0.5):
					y.append(float(dose))
				else:
					y.append(0.33)
	...
	if int(n) == 10:
		...
		plt.text(100,0.15,'Intervalor Contagem em Hora, inicio 25/05/10 fim 13/07/10')
		plt.title(u'São José dos Campos - Maio,Junho e Julho - Inverno_2010',fontsize=15)	
		plt.ylim(0.1,0.5)
