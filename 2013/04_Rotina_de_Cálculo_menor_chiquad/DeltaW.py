...
def raman1(x,Io,L,w0,wo,Gama):
	return Io*((np.exp(-(x**2)*((L)**2)/4))*(x**2)*4*np.pi)/(((w0-(wo-120*(x**2)))**2)+((Gama/2)**2))
...
for n in range(0,len(txts)):
	...
	os.chdir(principal)
	os.chdir('./Analise')
	V = open('./DeltaW_'+str(txts[n]),'w')
	V.write('DW(1/cm) L(nm) Gama(1/cm) Chi-quad\n')

	l=np.arange(5,100,5)
	g=np.arange(0.2,7,0.2)
	W=np.arange(520,525,0.5)
	...
	for pico in W:
		...
		for j in range(0,len(CHI)):
			...
			if CHI[j] == min(CHI):
				...
				gravar ='%2.2f   %4.2f   %2.2f   %f\n'%(Arm_wo[j]-520,Arm_L[j]*0.54,Arm_Gama[j],CHI[j])
				V.write(gravar)
				print gravar
		#Reset		
		Arm_L=[]
